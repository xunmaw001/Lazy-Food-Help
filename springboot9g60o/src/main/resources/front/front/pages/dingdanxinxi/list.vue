<template>
    ...
    <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
				...
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.dingdanzhuangtai" type="text" placeholder="订单状态" ></input>
			</view>
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.dingdanzhuangtai="";
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.dingdanzhuangtai){
			...
			async search(){
				...
				if(this.searchForm.dingdanzhuangtai){
					searchForm['dingdanzhuangtai'] = '%' + this.searchForm.dingdanzhuangtai + '%'
				}
				if(this.searchForm.yonghuxingming){
